...
module.exports = (serviceConfig, {
	...
	// Effect is an enum
	if(!effect)
		throw new Error(`Missing or empty 'effect' hook configuration in iamStatement hook: ${inspect(effect)}`);

	if(effect !== 'Allow' && effect !== 'Deny')
		throw new Error(`Invalid 'effect' hook configuration in iamStatement hook: ${inspect(effect)}`);

	// Action should be an array of strings
	if(!action || !action.length)
		throw new Error(`Missing or empty 'action' hook configuration in iamStatement hook: ${inspect(action)}`);

	const actionAsArray = Array.isArray(action) ? action : [action];
	if(actionAsArray.some(notAString))
		throw new Error(`Invalid 'action' hook configuration in iamStatement hook: ${inspect(action)}`);

	// Resource should be an array of strings
	if(!resource || !resource.length)
		throw new Error(`Missing or empty 'resource' hook configuration in iamStatement hook: ${inspect(resource)}`);

	const resourceAsArray = Array.isArray(resource) ? resource : [resource];
	if(resourceAsArray.some(notAString))
		throw new Error(`Invalid 'resource' hook configuration in iamStatement hook: ${inspect(resource)}`);
